...
def yolo_box_constructor(meta,np.ndarray[float] net_out, float threshold):
    # what is this meta? net_out is the np.concatenate((probs,confs),axis=0) array.
    cdef:
        float sqrt
        int C,B,S
    ...
    '''
    net_out is what we get from yolo net
    probs: the prob in each grid cell(S*S) of being class C
    confs: the confidence of B bbox in each cell(S*S)
    coords: the predicted coordinates(4) of bbox(B) in each grid cell(S*S)
    final_probs: all possible situation before NMS
    '''
    ...
    for grid in range(SS):
        for b in range(B):
            coords[grid, b, 0] = (coords[grid, b, 0] + grid %  S) / S #offset to each grid -> x coordinate w.r.t S
            coords[grid, b, 1] = (coords[grid, b, 1] + grid // S) / S #offset to each grid -> y coordinate w.r.t S
            coords[grid, b, 2] =  coords[grid, b, 2] ** sqrt # ?
            coords[grid, b, 3] =  coords[grid, b, 3] ** sqrt
            for class_loop in range(C):
                probs[grid, class_loop] = probs[grid, class_loop] * confs[grid, b] # P(cls|object)*[P(Object)*IOU]
